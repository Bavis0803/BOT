...
def perform_strategy(symbol, timeframe):
    ...
    if response.status_code == 200:
        ...
        # logic get type of candle
        # find imbalance candle
        condition_1 = (df['close'].shift(-1) < df['low']
                       ) & (df['high'].shift(-2) < df['low']) & (df.index < 997)  # type S
        condition_2 = (df['close'].shift(-1) > df['high']
                       ) & (df['low'].shift(-2) > df['high']) & (df.index < 997)  # type L
        df['imbalance'] = False
        df.loc[condition_1 | condition_2, 'imbalance'] = True
        df['imbalance_type'] = 'O'
        df.loc[condition_1, 'imbalance_type'] = 'S'
        df.loc[condition_2, 'imbalance_type'] = 'L'
        ######################################
        index_signal = None
        bot_signal = None
        ...
        if index_signal is not None:
            for index, row in df.loc[index_signal+1:].iterrows():
                if row['imbalance'] == True:
                    if bot_signal == 'L' and row['imbalance_type'] == 'L':
                        limit = row['high']
                        stoploss = row['low']
                        stoploss_percentage = (
                        ...
                        if symbol_dict[symbol] == 'L':
                            ...
                            send_message(symbol, 'L', timeframe,
                                         limit, tp, stoploss)
                        break
                    elif bot_signal == 'S' and row['imbalance_type'] == 'S':
                        limit = row['low']
                        stoploss = row['high']
                        stoploss_percentage = (
                        ...
                        if symbol_dict[symbol] == 'S':
                            ...
                            # call send mess function here
                            send_message(symbol, 'S', timeframe,
                                         limit, tp, stoploss)
                        break
        else:
            symbol_dict[symbol] == 'O'
    else:
